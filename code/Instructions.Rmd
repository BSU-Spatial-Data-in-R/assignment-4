...
Now that we've got our data aligned, Let's use a moving window to 'smooth' the data. Use the `focal()` function to take the mean of a 5x5 neighborhood and then a 10x10 neighborhood. This will go faster if you combine all of your aligned layers into a single `SpatRast` object - you should be able to do this by passing the names of your aligned rasters to `rast()`
